var markers = [];
var map = null;
var local_point = {lat:null, lng:null};
...
$('#local_btn').click(function( event ) {
    event.preventDefault();
    var select_lng = local_point.lng;
    var select_lat = local_point.lat;
    var select_sz = $('#size').val();
    var xmlHttp = new XMLHttpRequest();
    xmlHttp.onreadystatechange = function()
    {
...
function mark_search(tweet_array) {
    ...
    for(var _i=0;_i<tweet_array.length;_i++){
        ...
        var contentString = "<div  style='overflow: hidden;line-height: 1.35;width: 300px;'><div class=\"row\" >\n" +
            "        <div class=\"col-md-4\"><img src=\""+tweet_array[_i].headicon_url+"\"></div>\n" +
            "        <div class=\"col-md-8\"><h5>"+tweet_array[_i].author+"</h5></div>\n" +
            "    </div>\n" +
...
function initMap() {
	...
	map.addListener('click', function(e) {
	    local_point.lng = e.latLng.lng();
	    local_point.lat = e.latLng.lat();
		$('#mylongitude').val(local_point.lng.toString());
		$('#mylatitude').val(local_point.lat.toString());
  	});
}
